...
# 2. 환경설정
#######################################
STEP_1_3 = step_0.OUTPUT_DIR / "step_1_3.xlsx"
...
def main():
    ...
    df_copied["일"] = df_copied["일"].str.pad(width=2, side="left", fillchar="0")
    df_copied["거래일"] = df_copied["년"] + df_copied["월"] + df_copied["일"]
    ...
    df_filtered = df_merged.filter(["지역명", "거래일", "거래금액", "전용면적", "면적당금액", "아파트"])
    df_sorted = df_filtered.sort_values(["지역명", "거래일", "면적당금액", "아파트"], ascending=[True, True, False, True])
    df_grouped = df_sorted.groupby(["지역명"])
    df_result = df_grouped["면적당금액"].mean().to_frame().reset_index(drop=False)

    with pd.ExcelWriter(STEP_1_3) as writer:
