...
def main():
    ...
    gs_point = gdf_raw.geometry.representative_point()
    gdf_modified = gdf_raw.copy()
    gdf_modified["x"] = gs_point.x
    gdf_modified["y"] = gs_point.y

    df_price = pd.read_excel(step_2_1.STEP_2_1, sheet_name="result")
    df_price["시군구"] = df_price["지역명"].str.rsplit(n=1, expand=True).iloc[:, -1]
    df_price.columns = ["adm_nm", "avg_price", "sgg_nm"]

    gdf_merged: gpd.GeoDataFrame = pd.merge(gdf_modified, df_price, on="adm_nm", how="inner")
    gdf_merged = gdf_merged.filter(["adm_nm", "sgg_nm", "avg_price", "x", "y", "geometry"])
