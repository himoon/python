...
def main():
    ...
    set_font_style(run_title, face="나눔고딕", size=Pt(20), bold=True, color="0343DF")

    now_format = datetime.now().isoformat(sep=" ", timespec="minutes")
    run_datetime = p_title.add_run(f" ({now_format})")
    set_font_style(run_datetime, face="나눔고딕", size=Pt(14), bold=True, color="0343df")
    ...
    set_font_style(document.add_paragraph().add_run(" "), size=Pt(6))
    set_font_style(document.add_paragraph("").add_run("1. 주요 경제지표"), size=Pt(14), bold=True)
    set_font_style(document.add_paragraph().add_run(" "), size=Pt(10))
    ...
    with pd.ExcelFile(step_1_2.STEP_1_2) as xlsx:
        ...
        for idx, name in enumerate(sheet_names):
            ...
            img_path = step_1_3.STEP_1_3.as_posix().format(f"{name}M" if name == "기준금리" else name)
            ...
            set_font_style(p1.add_run(name), size=Pt(12), bold=True, color="333333")
            set_font_style(p2.add_run(f"{last_value:,.2f}"), size=Pt(14), bold=True, color="333333")
            set_font_style(p3.add_run(change), size=Pt(10), bold=True, color=rgb)
            ...
            set_font_style(p5.add_run(last_dt.strftime("%Y-%m-%d")), size=Pt(8), bold=True, color="888888")
